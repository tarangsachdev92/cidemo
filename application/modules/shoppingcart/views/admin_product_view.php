...
<div class="main-content">
    <div class="grid-data info-content">
        <div class="add-new">
            <?php echo anchor(site_url().$this->_data['section_name']. '/shoppingcart', lang('view_all_prd'), 'title="View All Products" style="text-align:center;width:100%;"'); ?>
        </div>
        <div class="tab-nav">
        ...
        <div class="profile-content-box" id="shoppingcart_order_view_form">
            <?php echo $content; ?>
        </div>
    </div>  
</div>
<script type="text/javascript">    
    $(document).ready(function()
    {
        ...
        load_form = function(lang_code)
        {
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url().$this->_data['section_name']; ?>/shoppingcart/view_product/'+lang_code+'/<?php echo $result[0]['products']['product_id']; ?>',
                data: {<?php echo $csrf_token; ?>: '<?php echo $csrf_hash; ?>'},
                success: function(msg) {
                    $("#shoppingcart_order_view_form").html(msg);
                }
            });
        }
    });
