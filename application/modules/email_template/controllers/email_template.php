 ...
 *
 *  Email Template controller to display Email Template page in front site.
 *   
 ...
 * @author AMPT
 */
class Email_Template extends Base_Front_Controller
{
    /*
     * Create an instance
    */
    ...
    {        
        ...
        //Logic  
        $cms_list = $this->email_template->get_cms_detail_by_slug($slug_url);

        if (count($cms_list) <= 0 || $this->input->post('select_language') != '')
        {
            $data_cms = $this->email_template->get_cms_id_from_slug_url($slug_url);
            if (count($data_cms))
            {
                $cms_list = $this->email_template->get_cms_detail_by_id($data_cms[0]['c']['cms_id'], $this->session->userdata[$this->section_name]['site_lang_id']);
                if (count($cms_list))
                {
    ...
    function defaultpage()
    {
        ...
        $data = array();           
        //Logic 
        $cms_list = $this->email_template->get_cms_default_page();
