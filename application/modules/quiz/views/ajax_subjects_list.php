    ...
    <div class="main-container">
        ...
        <div class="search-box">
            <table cellspacing="2" cellpadding="4" border="0">
                <tbody>
                    <tr>
                        <td align="right"><?php echo lang('search_by_subject_name') ?> :</td>
                        <?php
                        $input_data = array(
                            'name' => 'search_term',
                            'id' => 'search_term',
        ...
        <table cellspacing="1" cellpadding="4" border="0" bgcolor="#e6ecf2" width="100%">
            ...
            <?php
                ...
                foreach ($listing_data as $details)
                {
                    ...
                    <tr class="<?php echo $class; ?>">
                        ...
                        <td><?php echo $details['c']['category_title'] == '' ? 'None' : $details['c']['category_title']; ?></td>
                        <td>
                            <?php
                            if ($details['s']['status'] == '1')
                            {
                                ?>
                                <?php echo add_image(array('active.png')); ?>
                                <?php
                            }
                            elseif ($details['s']['status'] == '0')
                            {
                                ?> 
                        ...
                        <td>
                            <div class="action">
                                <div class="edit">
                                    <a href="<?php echo base_url().$this->_data['section_name']; ?>/quiz/subject_action/edit/<?php echo $details['l']['language_code'] . "/" . $details['s']['subject_id']; ?>" title="<?php echo lang('edit'); ?>">
                                        <?php echo add_image(array('edit.png')); ?>
                                    </a>
                                </div>

                                <div class="delete">
                                    <a href='javascript:;' title='<?php echo lang('delete'); ?>' onclick="delete_record('<?php echo $details['s']['subject_id']; ?>')">
            ...
            $options = array(
                ...
                'isAjaxRequest' => 1,
                'base_url' => base_url().$this->_data['section_name']."/quiz/ajax_subjects_list/" . $language_code,
...
<script>
    ...
    {
        if ($('#search_term').val() == '') {
        $('#search_term').validationEngine('showPrompt', '<?php echo lang('subject_name_required'); ?>', 'error');
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url().$this->_data['section_name']; ?>/quiz/ajax_subjects_list/<?php echo $language_code; ?>',
