<div class="main-container">
    <?php
    if($msg_no_category_records != '')
    {
        echo $msg_no_category_records;
        echo "<br>".anchor(base_url().$this->_data['section_name']."/quiz/category_action/add", lang('add_new_category'));
    }
    else
    {
    ?>
    <div class="grid-data grid-data-table">
        <div class="add-new">
            <span style="float: left;"><?php echo add_image(array('active.png'))."  Active ".add_image(array('inactive.png'))." Inactive";?></span>&nbsp;&nbsp;&nbsp;&nbsp;
                <?php echo anchor(base_url().$this->_data['section_name']."/quiz/category_action/add/" . $language_code, lang('add_new_category')); ?>
        </div>
        <div class="tab-nav">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        {
            res = confirm('<?php echo lang('msg_confirm_delete_category') ?>');
            ...
            if(res)
            {
                ...
                $.ajax({
                    type:'POST',
                    url:'<?php echo base_url().$this->_data['section_name']; ?>/quiz/delete_category',
                    data:{<?php echo $csrf_token; ?>:'<?php echo $csrf_hash; ?>',id:id},
                    success: function(response) {
                        unblockUI();
                        // show success message
                        alert('<?php echo lang('msg_success_delete_category') ?>');
                        var response = jQuery.parseJSON(response);

                        if(response.no_records == "TRUE")
                        {
                            window.location.href = '<?php echo base_url().$this->_data['section_name']; ?>/quiz/categories';
                        }
                        else
                        {
        ...
        load_ajax_index = function(lang_code) {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url().$this->_data['section_name']; ?>/quiz/ajax_categories_list/' + lang_code,
