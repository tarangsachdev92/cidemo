<div class="main-container">
    <?php
    if($msg_no_questions_records != '')
    {
        echo $msg_no_questions_records;
        echo "<br>".anchor(base_url().$this->_data['section_name']."/quiz/question_action/add", lang('add_new_question'));
    }
    else
    {
    ?>
    <div class="grid-data grid-data-table">
        <div class="add-new">
            ...
            <?php echo anchor(base_url().$this->_data['section_name']."/quiz/question_action/add/".$language_code, lang('add_new_question')); ?>
        </div>
        <div class="tab-nav">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        {
            res = confirm('<?php echo lang('msg_confirm_delete_question') ?>');
            ...
            if(res)
            {
                ...
                $.ajax({
                    type:'POST',
                    url:'<?php echo base_url().$this->_data['section_name']; ?>/quiz/delete_question',
                    data:{<?php echo $csrf_token; ?>:'<?php echo $csrf_hash; ?>',id:id},
                    success: function(response) {
                        unblockUI();
                        // show success message
                        alert('<?php echo lang('msg_success_delete_question') ?>');
                        var response = jQuery.parseJSON(response);

                        if(response.no_records == "TRUE")
                        {
                            window.location.href = '<?php echo base_url().$this->_data['section_name']; ?>/quiz/questions';
                        }
                        else
                        {
        ...
        load_ajax_index = function(lang_code) {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url().$this->_data['section_name']; ?>/quiz/ajax_questions_list/' + lang_code,
