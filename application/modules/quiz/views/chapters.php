<div class="main-container">
    <?php
    if($msg_no_chapters_records != '')
    {
        echo $msg_no_chapters_records;
        echo "<br>".anchor(base_url().$this->_data['section_name']."/quiz/chapter_action/add", lang('add_new_chapter'));
    }
    else
    {
    ?>
    <div class="grid-data grid-data-table">
        <div class="add-new">
            ...
            <?php echo anchor(base_url().$this->_data['section_name']."/quiz/chapter_action/add/".$language_code, lang('add_new_chapter')); ?>
        </div>
        <div class="tab-nav">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        {
            res = confirm('<?php echo lang('msg_confirm_delete_chapter') ?>');
            ...
            if(res)
            {
                ...
                $.ajax({
                    type:'POST',
                    url:'<?php echo base_url().$this->_data['section_name']; ?>/quiz/delete_chapter',
                    data:{<?php echo $csrf_token; ?>:'<?php echo $csrf_hash; ?>',id:id},
                    success: function(response) {
                        unblockUI();
                        // show success message
                        alert('<?php echo lang('msg_success_delete_chapter') ?>');
                        var response = jQuery.parseJSON(response);

                        if(response.no_records == "TRUE")
                        {
                            window.location.href = '<?php echo base_url().$this->_data['section_name']; ?>/quiz/chapters';
                        }
                        else
                        {
        ...
        load_ajax_index = function(lang_code) {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url().$this->_data['section_name']; ?>/quiz/ajax_chapters_list/' + lang_code,
